...
if (!function_exists('cbia_register_heartbeat_script')) {
    function cbia_register_heartbeat_script() {
        if (!wp_script_is('heartbeat', 'registered')) {
            wp_register_script('heartbeat', includes_url('js/heartbeat.min.js'), array('jquery'), CBIA_VERSION, true);
        }
    }
    add_action('init', 'cbia_register_heartbeat_script', 1);
}
