...
/**
 * CatÃ¡logo (para UI y coherencia v8.4)
 */
if (!function_exists('cbia_image_formats_catalog')) {
    function cbia_image_formats_catalog(): array {
        return [
            'panoramic_1536x1024' => [
                'label' => 'PanorÃ¡mica (1536x1024)',
                'size'  => '1536x1024',
                'type'  => 'panoramic',
            ],
            'banner_1536x1024' => [
                'label' => 'Banner (1536x1024, encuadre amplio + headroom 25â€“35%)',
                'size'  => '1536x1024',
                'type'  => 'banner',
            ],
        ];
...
if (!function_exists('cbia_get_image_format_for_section')) {
     ...
     * - destacada/intro => panorÃ¡mica
     * - resto => banner
     * Aunque haya settings guardados, aquÃ­ se fuerza por compatibilidad.
     */
...
if (!function_exists('cbia_build_image_prompt')) {
    function cbia_build_image_prompt($desc, $section, $title) {
        ...
        if ($desc !== '') $prompt .= " DescripciÃ³n: {$desc}.";

        if (cbia_is_banner_format($fmt)) {
            $prompt .= " ComposiciÃ³n tipo banner con espacio superior (headroom).";
        } else {
            $prompt .= " ComposiciÃ³n panorÃ¡mica equilibrada.";
        }
