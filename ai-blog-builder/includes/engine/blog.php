 ...
 * 1) Si cbia_log_message NO estÃƒÂ¡ cargada (por orden de includes),
 *    este archivo ahora trae un logger Ã¢â‚¬Å“fallbackÃ¢â‚¬Â para que SIEMPRE haya log.
 * 2) El botÃƒÂ³n "Crear Blogs" ahora ARRANCA de verdad:
 *    - Ejecuta 1 tanda inmediata (para que veas log al instante)
 *    - y re-encola el evento para continuar en background si queda cola.
 * 3) Log Ã¢â‚¬Å“en vivoÃ¢â‚¬Â:
 *    - AÃƒÂ±ade contador anti-cache y nocache_headers()
 *    - Endpoint wp_ajax_cbia_get_log se registra ahora en core/hooks.php.
 *
 * IMPORTANTE:
 * - Si tu hosting tiene WP-CRON bloqueado, al menos verÃƒÂ¡s la primera tanda,
 *   y podrÃƒÂ¡s re-lanzar con el botÃƒÂ³n para seguir.
 */
...
if (!function_exists('cbia_log_message')) {
    function cbia_log_message($message) {
        ...
        // error_log eliminado en FREE (evitar warnings en revisiÃ³n).
    }
}

// cbia_clear_log y cbia_get_log viven en el nÃƒÂºcleo.
...
if (!function_exists('cbia_create_all_posts_checkpointed')) {
    function cbia_create_all_posts_checkpointed($incoming_titles=null, $max_per_run = 1){
        ...
        if (!$incoming_titles && !empty($cp) && !empty($cp['running']) && isset($cp['queue']) && is_array($cp['queue'])) {
            ...
            if (empty($titles)) {
                cbia_log_message("[INFO] Sin tÃƒÂ­tulos. Fin.");
                return array('done'=>true,'processed'=>0);
            }
        ...
        if (empty($queue)) {
            cbia_log_message("[INFO] No hay tÃƒÂ­tulos nuevos. Fin.");
            cbia_checkpoint_clear();
            return array('done'=>true,'processed'=>0);
        }
        ...
        foreach ($queue as $i => $title) {
            ...
            if ($processed_this_run >= $max_per_run) {
                cbia_log_message("[INFO] Tanda completada: processed_this_run={$processed_this_run}. Se continuarÃƒÂ¡ en el siguiente evento.");
                break;
            }
        }
...
if (!function_exists('cbia_run_generate_blogs')) {
    function cbia_run_generate_blogs($max_per_run = 1){
        ...
        cbia_log_message("[INFO] Iniciando creaciÃƒÂ³n de blogs (checkpoint)Ã¢â‚¬Â¦");
...
if (!has_action('cbia_generation_event')) {
    add_action('cbia_generation_event', function () {
        cbia_log_message('[INFO] Ejecutando tanda en evento (background)Ã¢â‚¬Â¦');
