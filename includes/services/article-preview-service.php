...
if (!class_exists('CBIA_Article_Preview_Service')) {
    class CBIA_Article_Preview_Service {
        ...
        private function generate_internal(array $payload, $emit = null) {
            ...
            $used_stream = false;
            if (is_callable($emit) && function_exists('cbia_openai_responses_stream_call')) {
                list($ok, $text_html, $usage, $model_used, $err) = cbia_openai_responses_stream_call(
                    $prompt,
                    $title,
                    2,
                    function($delta, $acc_html) use ($emit) {
                        $this->emit($emit, 'text_progress', array(
                            'html' => (string)$acc_html,
                            'word_count' => $this->word_count((string)$acc_html),
                        ));
                    }
                );
                if ($ok) {
                    ...
                    $this->emit($emit, 'cbia_status', array(
                        'message' => 'Streaming no disponible, continuando en modo clasico...'
                    ));
                }
            }
            if (!$used_stream) {
            ...
            $this->emit($emit, 'cbia_content', array('html' => $text_html));
            $this->emit($emit, 'word_count', array('count' => $this->word_count($text_html)));
            if (is_callable($emit) && !$used_stream) {
        ...
        public function create_post_from_token($token, array $overrides = array()) {
            ...
            if (function_exists('error_log')) {
                error_log('[CBIA_PREVIEW_TMP] token consume attempt token=' . $token);
            }
            $user_id = get_current_user_id();
        ...
        private function store_preview_payload($user_id, array $payload): string {
            ...
            if (function_exists('error_log')) {
                error_log('[CBIA_PREVIEW_TMP] token generated user=' . $user_id . ' token=' . $token);
            }
            return $token;
        }
