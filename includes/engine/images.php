...
if (!function_exists('cbia_get_image_format_for_section')) {
     ...
     * - destacada/intro => panoramica
     * - resto => banner
     * Aunque haya settings guardados, aqui se fuerza por compatibilidad.
     */
...
if (!function_exists('cbia_build_content_img_tag_with_meta')) {
     ...
     */
    function cbia_build_content_img_tag_with_meta($url, $alt, $section, $attach_id = 0): string {
        $tag = cbia_build_content_img_tag($url, $alt, $section);
...
if (!function_exists('cbia_default_image_prompt_template')) {
    function cbia_default_image_prompt_template(): string {
        return "Professional editorial photography. Subject: {desc} related to {title}. {format} No text, no logos, no watermarks.";
    }
}

if (!function_exists('cbia_get_image_prompt_template')) {
    function cbia_get_image_prompt_template(): string {
        $s = function_exists('cbia_get_settings') ? cbia_get_settings() : [];
...
if (!function_exists('cbia_image_prompt_format_hint')) {
    function cbia_image_prompt_format_hint($section): string {
        $fmt = cbia_get_image_format_for_section($section);
        if (cbia_is_banner_format($fmt)) {
            return 'Wide banner composition with generous headroom and negative space.';
        }
        return 'Wide panoramic composition with balanced framing and negative space.';
    }
}
...
if (!function_exists('cbia_format_image_prompt')) {
    function cbia_format_image_prompt($tpl, $desc, $title, $section, $type, $format_hint): string {
        ...
        return str_replace(array_keys($replacements), array_values($replacements), (string)$tpl);
    }
}
...
if (!function_exists('cbia_build_image_prompt_for_post')) {
    function cbia_build_image_prompt_for_post($post_id, $type, $short_desc, $idx = 0): string {
        ...
        if ($short_desc === '') {
            ...
            cbia_log(__('Imagen: SHORT_DESC vacio, usando fallback por titulo', 'ai-blog-builder-pro'), 'INFO');
        }

        $override = cbia_get_img_prompt_override($post_id, $type, $idx);
        if ($override !== '') {
            cbia_log(sprintf(__('Imagen: usando override de prompt (%s%s)', 'ai-blog-builder-pro'), (string)$type, $idx ? '_' . (string)$idx : ''), 'INFO');
            return $override;
        }

        $section = ($type === 'featured') ? 'intro' : 'body';
        $tpl = cbia_get_image_prompt_template();
