...
if (!defined('CBIA_TEXT_DOMAIN')) define('CBIA_TEXT_DOMAIN', 'ai-blog-builder-pro');
...
add_action('plugins_loaded', function () {
	load_plugin_textdomain(CBIA_TEXT_DOMAIN, false, dirname(plugin_basename(__FILE__)) . '/languages');
});

if (!function_exists('abb_pro_is_free_active')) {
	function abb_pro_is_free_active(): bool {
...
if (!abb_pro_is_free_active()) {
	add_action('admin_notices', function () {
		...
		echo '<strong>AI Blog Builder Pro:</strong> requiere que <em>AI Blog Builder (FREE)</em> esté activo.';
		echo '</p></div>';
	});
	return;
}
...
$cbia_bootstrap = CBIA_PRO_INCLUDES_DIR . 'core/bootstrap.php';
...
if (!function_exists('cbia_log')) {
	...
	function cbia_log(string $message, string $level = 'INFO'): void {
		...
		if (defined('CBIA_TEXT_DOMAIN')) {
			$message = __((string)$message, CBIA_TEXT_DOMAIN);
		}
...
foreach ($cbia_modules as $f) {
	if (file_exists($f)) {
		...
		cbia_log(__('','ai-blog-builder-pro') . basename($f), 'ERROR');
	}
}
